...
# # # Database and camera configurations
# # database = {
# #     "6jbhush87njeu8": {"name": "Pratham", "url": "https://res.cloudinary.com/dcbrfuldz/image/upload/v1732727490/YelpCamp/coehrxur5ohi2tcstkrx.jpg"},
# # }
...
# # # Global variables for managing camera threads and stop signals
# # stop_signal = Event()
# # camera_threads = []
...
# # def process_camera(camera, detected_results, known_face_encodings, known_face_names):
# #     camera_id = camera["url"]
# #     cap = cv2.VideoCapture(camera_id)
# #     frame_count = 0
# #     process_every_nth_frame = 45
...
# #         frameS = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
# #         if frame_count % process_every_nth_frame == 0:
# #             print('Known face names inside process camera', known_face_names)
# #             face_locations = face_recognition.face_locations(frameS, model="hog")
# #             face_encodings = face_recognition.face_encodings(frameS, face_locations, 1, "large")
# #             face_names = []
# #             for face_encoding, faceLoc in zip(face_encodings, face_locations):
...
# #         if cv2.waitKey(1) & 0xFF == ord('q'):
# #             break
# #     print(f"Exiting process_camera for {camera_id}")
# #     cap.release()
# #     cv2.destroyWindow(f'Camera {camera_id}')
# # # Start detecting known persons
# # def detect_known_persons():
# #     global camera_threads
# #     stop_camera_threads()  # Stop previous threads
# #     stop_signal.clear()
# #     known_face_encodings, known_face_names = load_known_faces(database)
# #     detected_results = {}
# #     print('know_face_names inside detect_known_person before starting the process_camera thread ', known_face_names)
# #     print("Starting camera threads...")
# #     for camera in cameras:
# #         thread = Thread(target=process_camera, args=(camera, detected_results, known_face_encodings, known_face_names))
# #         thread.daemon = True
# #         thread.start()
# #         camera_threads.append(thread)
# #     print(f"{len(camera_threads)} camera threads started.")
# # # Stop all camera threads
# # def stop_camera_threads():
# #     print("Stopping camera threads...")
# #     stop_signal.set()
# #     # print(f"Active threads before join: {Thread.enumerate()}")
# #     for thread in camera_threads:
# #         thread.join()
# #     print("All threads joined.")
# #     camera_threads.clear()
# #     cv2.destroyAllWindows()
# #     print(f"{len(camera_threads)} camera threads are running.")
# #     # print(f"Active threads after join: {Thread.enumerate()}")
...
# # cameras = [{"name": 'varun', "url": "http://192.168.62.157:4747/video", "camId": 'tdwhgd'}]
...
# # @csrf_exempt
# # def add_face(request):
# #     global detection_thread,stop,database
...
# #             # stop.clear()
# #             # detection_thread = Thread(target = detect_known_persons,daemon = True)
...
# #         for reportid in list(database.keys()):
# #             name = reportid
# #             img_path = database[reportid]["url"]
# #             if not img_path or not isinstance(img_path, str):
# #                 print(f"Invalid path for {name}: {img_path}")
...
# # def detect_known_persons(tolerance=0.5):
# #     global stop 
# #     def process_camera(camera,known_face_names,known_face_encodings, stop_signal):
        
# #         print("Process camera started")
# #         camera_id = camera["url"]
# #         cap = cv2.VideoCapture(camera_id)
# #         frame_count = 0
# #         process_every_nth_frame = 45 
...
# #             if frame_count % process_every_nth_frame == 0:
# #                 print(f"Stop Signal: {stop_signal[0]} | Frame Count: {frame_count}")
...
# #         print("faces are loaded",known_face_names)
# #         for camera in cameras:
# #             process_camera(camera,known_face_names,known_face_encodings,stop_signal)
...
@csrf_exempt
def get_missing_person_request(request):
    if request.method == "GET":
...
async def add_face(request):
    if request.method == "GET":
        ...
        try:
            try:
                ...
                async with aiohttp.ClientSession() as session: 
                    async with session.post(url, json=payload) as response: 
                        ...
                        print("Updated value of flag at start of add_face function is", flag)
            ...
            # print('known_face_names inside add_face after saving ', known_face_names)
            # if not name or not url:
            #     return JsonResponse({"error": "Name and URL are required."}, status=400)
            ...
            try: 
                ...
                async with aiohttp.ClientSession() as session: 
                    async with session.post(url, json=payload) as response: 
                        ...
                        print("Updated value of flag at start of add_face function is", flag)
                flag = response_data["flag"]
                print("Updated value of flag at end of add_face function is", flag)
            except requests.RequestException as e:
                print(f"RequestException occurred: {e}")
...
async def detect_known_persons( tolerance=0.5):
    ...
    def run_async_tasks():
        ...
        # Creating tasks
        tasks = [
            process_camera(camera, detected_results, face_encodings, face_names, stop_signal)
            for camera in cameras
        ]
        tasks.append(stop_on_input(stop_signal))
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.submit(run_async_tasks)
